...
export class RealOnboardingDataSource implements OnboardingDataSource {
  ...
  async saveStep(form: OnboardingFormState): Promise<void> {
    const response = await fetch("/api/v1/onboarding/draft", {
      method: "PUT",
      credentials: "same-origin",
      headers: { "content-type": "application/json" },
      body: JSON.stringify(formStatePatch(form))
    });
    if (!response.ok) {
...
function formStatePatch(form: OnboardingFormState) {
  const { clientOperationId: _clientOperationId, baseVersion: _baseVersion, ...patch } = form;
  return patch;
}
