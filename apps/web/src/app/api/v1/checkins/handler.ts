...
export async function submitCheckinResponse(request: Request, user: SessionUser, database: PostgresJsDatabase<typeof schema>): Promise<Response> {
  ...
  try {
    ...
    const recommendation = await repository.submitCheckin(user.id, checkinDate ?? isoDate(), input);
